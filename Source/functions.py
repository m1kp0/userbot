...
async def google_translate_command(msg): # тут есть небольшой баг, бот пока что не может перевести ответы на другой язык.
    reply = await get_reply(msg)
    tr = Translator()
    try:
        if msg.is_reply:
...
async def silent_spam_command(msg):
    ...
    try:
        ...
        await edit_msg(msg, "Сообщения отправляются..")
        for i in range(int(text[1])):
            silent_msg = await cl.m1kp.send_message(msg.chat_id, text_to_sms) # Не работает через функцию send_msg
            await sleep(0.4)
